...
# export USE_HADOOP=                            # Whether include hadoop jars into zeppelin server process. (true or false)
 export SPARK_MASTER=spark://spark-master:7077  # Spark master url. eg. spark://master_addr:7077. Leave empty if you want to use local mode.
 export ZEPPELIN_ADDR=0.0.0.0                        # Bind address (default 127.0.0.1)
 export ZEPPELIN_PORT=4070                       # port number to listen (default 8080)
...
export SPARK_CONF_DIR  ./spark/conf/                       # (optional) In the zeppelin interpreter on docker mode, Need to set the local spark conf folder path
